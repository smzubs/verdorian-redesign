...
> ## ⚡ LATEST (2026-07-12) — SERVICES CARDS + PROBLEM SECTION + HERO SUBHEAD REBUILT, LIVE HEAD `6cd7579` — read first
> **PAGE ORDER (LIVE `162ffc7`):** Nav → Hero → MatrixBand → PainSection(#problem, statement band) → **Capabilities(#automate, What We Do)** → **Pricing(#pricing)** → Trust(#trust) → bridge → Contact → Footer. Nav/Footer tabs synced to doc order: What We Do / Pricing / Our Work (scrollspy verified).
> **PROBLEM SECTION → ONE STATEMENT BAND — user-directed, LIVE `2988fa4`:** PainSection(#problem) is now ONLY the big premium line "If it repeats every day, every week — *it can be automated.*" (Cormorant clamp 34–64px, accent = `.gold-shimmer` blue italic block line). Heading/sub/struck-words/3-node strip ALL DELETED (recover via git — they lived one commit, `d0dc9a9`). Section id kept.
> **$500 PRICE MENTIONED ONLY IN PRICING (#pricing) — user-directed, LIVE `6b0db28`:** stripped from Nav mobile CTA ("Start Your Test-Drive"), Contact reassurance bullet ("The pilot fee is fully credited…"), and /trades ("The pilot fee is credited…"). Terms page KEEPS the $500 (legal accuracy). Verified live: all 3 homepage $500 occurrences are inside #pricing.
> **HERO IS TEXT-ONLY AGAIN (user-directed):** pilot-mechanic subhead ("two weeks… pay if it hits the number"), the **Start Your $500 Test-Drive CTA button**, and the micro-trust line ("no IT department… Fully credited") were ALL DELETED from Hero. New Fraunces subhead: "We build custom automation and software for small & medium enterprises— / eliminating repetitive office work so your team runs **fast, smart, and right.**" (triad blue 600; user's "100% Right way" softened to "right" per no-100%-claims honesty rule — flagged, he can reinstate). **First CTA on the page is now Pricing's** — GlowButton/scrollToSection imports dropped from Hero. $500 Test-Drive offer still lives in Pricing + Contact.
> **PainSection (#problem) rewritten to one concept** (user: shorter/simpler/smarter): heading "Most busywork is moving information from one place to another. / That's a job for software — not your team." → one-sentence sub (email→spreadsheet row, form→invoice) → struck-through words row (copy·paste·retype·remind·chase, blue strikethrough) → ONE glass strip with 3 nodes (Work arrives → **Software runs the middle** [blue accent] → You approve) → italic closer "If it repeats every week, it can run itself." **Dual manual-vs-automated lanes + People×Hours×Repetition formula DELETED** (recover via git). Mobile stacks vertically w/ rotated arrows. QA'd live 1280+375, 0 errors, no overflow. Playwright gotcha: the looping `.pf-arrow` animation makes element-screenshots time out ("element is not stable") — use viewport screenshots for this section.
